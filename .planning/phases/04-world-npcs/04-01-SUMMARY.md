...
dependency_graph:
  requires:
    - "Core/Entity.cs (base class)"
    - "Core/GameState.cs (QuestState int field pre-existed since v3)"
    - "Inventory/InventoryManager.cs (existing slot/equip API)"
    - "World/TileMap.cs (TMX object layer parsing pattern)"
  provides:
...
key-files:
  created:
    - "Quest/MainQuestState.cs"
    - "Quest/MainQuest.cs"
    - "Entities/NpcEntity.cs"
    - "World/TriggerZone.cs"
  modified:
    - "World/TileMap.cs"
...
// Entities/NpcEntity.cs : Core.Entity
...
// Inventory/InventoryManager.cs (additions)
...
// Core/ServiceContainer.cs (additions)
...
- Existing farm map (`Content/Maps/test_farm.tmx`) has no `Triggers` object group; `TileMap.Triggers` returns empty list -- baseline unchanged.

## Self-Check: PASSED

- Quest/MainQuestState.cs: FOUND (contains `NotStarted = 0` and `Complete = 2`)
- Quest/MainQuest.cs: FOUND (contains `public event Action<MainQuestState>? OnQuestStateChanged`)
- Entities/NpcEntity.cs: FOUND (contains `public const float InteractRange = 28f`)
- World/TriggerZone.cs: FOUND (contains `public record TriggerZone(string Name, Rectangle Bounds)`)
- World/TileMap.cs: modified (contains `LoadTriggerObjects` + `public IReadOnlyList<TriggerZone> Triggers`)
- Inventory/InventoryManager.cs: modified (contains `TrySpendGold`, `OnGoldChanged`, Gold round-trip)
- Core/ServiceContainer.cs: modified (contains `public MainQuest? Quest`)
- Core/SaveManager.cs: modified (contains `CURRENT_SAVE_VERSION = 5` and `state.SaveVersion < 5` branch)
- Scenes/FarmScene.cs: modified (contains `Services.Quest = _mainQuest`)
