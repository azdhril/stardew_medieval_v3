...
dependency_graph:
  requires: [04-04]
  provides: ["Live in-memory Inventory/Quest survives Farm re-entry"]
  affects: ["Scenes/FarmScene.cs OnLoad lifecycle"]
tech_stack:
  added: []
...
key_files:
  ...
  modified:
    - "Scenes/FarmScene.cs"
decisions:
  - "Narrow fix: only gate Inventory/Quest/Time/Stamina/Player-position behind firstEntry. Farm cells still re-hydrate from disk every entry (existing behavior, not in scope of UAT Test 13 blocker)."
...
### Scenes/FarmScene.cs (1 file, 1 task)
...
| 5 | Esc shop → walk Village → Farm. | Returns to Farm scene. | **FIXED** — Inventory/Quest no longer wiped on re-entry |
...
- `grep -n "Services.Inventory == null" Scenes/FarmScene.cs` → 1 hit (line 86).
- `grep -n "bool firstEntry" Scenes/FarmScene.cs` → 1 hit (line 86).
- `grep -n "firstEntry && save == null" Scenes/FarmScene.cs` → 1 hit (line 172).
- `grep -n "firstEntry &&" Scenes/FarmScene.cs` → 2 hits (lines 172, 183).
...
- `Scenes/FarmScene.cs` — modified, present, builds clean (Debug + Release).
